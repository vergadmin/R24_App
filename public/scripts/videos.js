...
var videoInfo = {
    ...
    "percentWatched": '0',
    "videoPlayedSegments": {},
    "videoTimeWatchedInSeconds": 0,
    "videoTotalDurationInSeconds": 0,
    "timeEnterPage": '',
    "timeExitPage": '',
...
window.onload = function() {
    videoInfo["timeEnterPage"] = getLocalTime("enter")
    textInfo["timeEnterPage"] = getLocalTime("enter")
};
  

window.addEventListener("beforeunload", (event) => {
    if (sessionStorage.getItem("vCHE") ==='text') {
        textInfo["vCHE"] = "text"
        textInfo["timeExitPage"] = getLocalTime("exit")
        const timeSpentMs = exitTimeObject - enterTimeObject;
        ...
        player.getDuration().then(function(duration) {
            ...
            videoInfo["videoTotalDurationInSeconds"] = duration
            console.log("VIDEO INFO", videoInfo)
            videoTotalDuration = duration
            player.getPlayed().then(function(played) {
                videoInfo["videoPlayedSegments"] = played
                ...
                videoInfo["videoTimeWatchedInSeconds"] = played
                var percentageWatched = Math.round((totalTime / videoTotalDuration) * 100);
                videoInfo["percentWatched"] = percentageWatched + "%"
                console.log("VIDEO INFO", videoInfo)
        
                const timeSpentMs = exitTimeObject - enterTimeObject;
                const timeSpentSeconds = Math.floor(timeSpentMs / 1000);
                videoInfo["pageDurationSeconds"] = timeSpentSeconds
                
                logVideosToDatabase(videoInfo)
